...
function inferAreaFromText(text, city, district) {
    ...
    const inferredDistrict = (inferredCity && taiwanAreas[inferredCity]?.find((item) => normalizeTaiwanText(district) === item)) ||
...
export function sanitizeImportAnalysis({ raw, normalizedUrl, sourcePlatform, sourceCaption, warnings = [], }) {
    const area = inferAreaFromText(`${sourceCaption}\n${toStringValue(raw.address)}`, toStringValue(raw.city), toStringValue(raw.district));
    const aiDailyMealTags = unique(toStringArray(raw.aiDailyMealTags).filter((tag) => dailyMealTags.includes(tag)));
    const aiCuisineTags = unique(toStringArray(raw.aiCuisineTags).filter((tag) => cuisineTags.includes(tag)));
    const tags = unique([...toStringArray(raw.tags), ...aiDailyMealTags, ...aiCuisineTags].filter((tag) => allTags.includes(tag)));
    const result = {
    ...
    if (result.missingFields.length)
        result.aiConfidence = Math.min(result.aiConfidence ?? 0.5, 0.55);
    return result;
}
