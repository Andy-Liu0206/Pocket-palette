...
export function extractCaptionFromSharedText(value: string) {
  return value
    .replace(/https?:\/\/[^\s<>"'，。)）]+/gi, '')
    .split('\n')
    .map((line) => line.trim())
...
export function extractCaptionFromMetadata(html: string, platform: SourcePlatform) {
...
export async function fetchCaptionFromUrl(url: string, platform: SourcePlatform) {
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: {
        'User-Agent':
          'Mozilla/5.0 (compatible; PocketPaletteBot/0.1; +https://pocket-palette-ai.onrender.com)',
