...
export async function analyzeRestaurantWithOpenRouter({
  caption,
  url,
  platform,
}: {
        ...
        messages: [
          {
            role: 'system',
            content:
              '你是 Pocket Palette 的台灣美食貼文解析器。只根據使用者提供的文案與連結輸出 JSON，不要編造缺少的資訊。所有輸出使用繁體中文。若欄位無法判斷，使用空字串或空陣列，並在 warnings 說明。',
          },
          {
            role: 'user',
