import { allTags, categoryGroups } from '../data/categories';
import { taiwanAreas } from '../data/taiwanAreas';
import type { Restaurant, SourcePlatform } from '../types/restaurant';

declare const process: { env?: Record<string, string | undefined> } | undefined;

const dailyMealTags = [...categoryGroups[0].tags];
const cuisineTags = [...categoryGroups[1].tags];
...
function extractCaptionFromSharedText(value: string) {
  return value
    .replace(/https?:\/\/[^\s<>"'，。)）]+/gi, '')
    .split('\n')
    .map((line) => line.trim())
...
export function normalizeSocialUrl(url: string) {
  ...
  try {
    ...
    if (!shouldPreserveSearchParams(parsed)) parsed.search = '';
    parsed.hash = '';
...
export function detectSourcePlatform(url: string): SourcePlatform {
  ...
  if (lowered.includes('threads')) return 'Threads';
  if (lowered.includes('/reel/')) return 'Reels';
  if (lowered.includes('instagram')) return 'Instagram';
  if (lowered.includes('maps.google') || lowered.includes('goo.gl/maps') || lowered.includes('google.com/maps')) return 'Google Maps';
  if (lowered.includes('http')) return 'Website';
  return 'Other';
}
...
function readMetaContent(html: string, key: string) {
  ...
  for (const tag of metaTags) {
    ...
    if (content?.[2]) return decodeHtml(content[2]);
  }
  return '';
}

function extractCaptionFromMetadata(html: string, platform: SourcePlatform) {
  ...
  if (platform === 'Instagram' || platform === 'Reels' || platform === 'Threads') {
    ...
    if (descriptionQuoted?.[1]) return descriptionQuoted[1].trim();
...
async function fetchCaptionFromUrl(url: string, platform: SourcePlatform) {
  const response = await fetch(url);
  if (!response.ok) throw new Error(`Unable to fetch URL metadata: ${response.status}`);
...
function normalizeBackendAnalysis(
  payload: BackendImportAnalysis,
  normalizedUrl: string,
  sourcePlatform: SourcePlatform,
): SocialImportAnalysis | undefined {
  ...
  if (!result.name && !sourceCaption) return undefined;
...
async function analyzeWithConfiguredBackend(
  normalizedUrl: string,
  caption: string,
  sourcePlatform: SourcePlatform,
): Promise<SocialImportAnalysis | undefined> {
  ...
  try {
    ...
    const payload = (await response.json()) as BackendImportAnalysis;
...
function cleanDecorativeText(value: string) {
  return value
    .replace(/[@#].*$/, '')
    .replace(/[˚₊୧⃛୨⃛✧˖◛⁺✰჻ᐟ☆★|｜·•・]/g, ' ')
...
function extractRestaurantName(caption: string) {
  const patterns = [
    /(?:店名|餐廳|店家)\s*[:：｜|]\s*([^\n#]+)/,
    /-\s*\n\s*📌?\s*([^\n#]+)/,
  ...
  for (const pattern of patterns) {
    ...
    if (match?.[1]) {
      ...
      if (!candidate.toLowerCase().startsWith('ig') && !candidate.includes('@')) return candidate;
    }
  }
...
function extractArea(caption: string, address: string) {
  const haystack = `${caption}\n${address}`.replace(/臺/g, '台');
  const city = Object.keys(taiwanAreas).find((item) => haystack.includes(item)) ?? '';
  const district = city ? taiwanAreas[city].find((item) => haystack.includes(item)) ?? '' : '';
...
function extractSignatureFoods(caption: string) {
...
function inferDailyMeals(caption: string) {
  const tags: string[] = [];
  ...
  return unique(tags.filter((tag) => (dailyMealTags as readonly string[]).includes(tag))).slice(0, 3);
}

function inferCuisineTags(caption: string) {
...
function extractTags(caption: string, dailyMeals: string[], cuisines: string[]) {
  let directTags = allTags.filter((tag) => caption.includes(tag));
  ...
  if (caption.includes('咖啡') && !situationalTags.includes('咖啡廳')) situationalTags.push('咖啡廳');
  return unique([...dailyMeals, ...cuisines, ...directTags, ...situationalTags]).slice(0, 8);
}

function buildMissingFields(result: Partial<Restaurant>) {
...
export async function analyzeSocialFoodUrl(url: string, caption = ''): Promise<SocialImportAnalysis> {
